	   ...
	   <div class="z-news-con">
	       ...
	       <div class="z-news-con-l">
		       ...
		       @if(isset($zhi) && count($zhi) > 0)
			      <li class="z-n-li0"><b>{{ $zhi->title }}</b><p><span class="z-n-ps1">{{ date("Y年m月d日 H:i:s",$zhi->time) }}</span><span class="z-n-ps2">{{ $zhi->click }}</span></p><img src="{{ asset('/uploads/plate/newsimg/') }}/{{ $zhi->titleimg }}" width="640" height="250" /><span class="z-n-ps3">{{ $zhi->leicon }}</span><a href="{{ url('/home/plate/play/') }}/{{ $zhi->id }}">查看全文</a></li>
		       @endif
