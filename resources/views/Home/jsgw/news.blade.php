	   ...
	   <div class="z-news-con">
	       ...
	       <div class="z-news-con-l">
		       ...
		       @if(isset($zhi) && count($zhi) > 0)
			      <li class="z-n-li0"><b>{{ $zhi->title }}</b><p><span class="z-n-ps1">{{ date("Y年m月d日 H:i:s",$zhi->time) }}</span><span class="z-n-ps2">{{ $zhi->click }}</span></p><img src="{{ asset('/uploads/news/titleimg/') }}/{{ $zhi->titleimg }}" width="640" height="250" /><span class="z-n-ps3">{{ mb_substr($zhi->leicon,0,127,'utf8').'…'  }}</span><a href="{{ url('/news/list/') }}/{{ $zhi->id }}">查看全文</a></li>
		       @endif
	       ...
	       <div class="z-news-con-r">
		      <div class="z-news-r-1">
			     ...
			     @if(isset($qi) && count($qi) > 0)
				    ...
				    <li><img src="{{ asset('uploads/news/img/') }}/{{ $vvv->img }}" width="120" height="60"/><a href="{{ url('/news/') }}/{{ $vvv->id }}"><b>{{ $vvv->title }}</b></a><span></span></li>
				 	@endforeach
				 @else
